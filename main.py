...
if not any(value is "" for value in continuous_inputs.values()):
    normalized_continuous_vars = normalize(x=pd.DataFrame(continuous_inputs, index=[0]), x_original=pd.DataFrame(continuous_vars, index=[0]))
